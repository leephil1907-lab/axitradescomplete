    ...
    <div className="w-full flex flex-col gap-2">
      ...
      >
        {formattedData.length === 0 ? (
          <div style={{ height: chartHeight }} className="flex flex-col items-center justify-center text-center p-6 space-y-2">
            <Activity className="w-8 h-8 text-slate-600 animate-pulse" />
            <p className="text-sm font-bold text-slate-400">Awaiting Real-Time OHLC History Feed</p>
            <p className="text-xs text-slate-600 max-w-sm">
              Exchange historical candle data for <span className="font-mono text-brand-yellow">{symbol}</span> ({timeframe}) will render directly upon receiving verified provider intervals.
            </p>
          </div>
        ) : (
