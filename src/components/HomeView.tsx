...
import { MASTER_TRADERS, ACCOUNT_TYPES, DEFAULT_MARKET_QUOTES } from '../data';
...
function MobileAppPreview({ quotes, onAddFunds }: { quotes: Record<string, MarketQuote>; onAddFunds: () => void }) {
  const [activeTab, setActiveTab] = useState<'cfds' | 'perps' | 'crypto'>('cfds');
  const [selectedAsset, setSelectedAsset] = useState<string>('BTC/USD');
  const [equity] = useState<number>(5301.23);
  const [pnl] = useState<number>(32.11);

  const prices = React.useMemo(() => {
    return {
      'BTC/USD': { 
        val: quotes['BTCUSD']?.price || 96450, 
        chg: quotes['BTCUSD']?.change || 2.85, 
        isUp: (quotes['BTCUSD']?.change || 0) >= 0 
      },
      'SOL/USD': { 
        val: quotes['SOLUSD']?.price || 188.40, 
        chg: quotes['SOLUSD']?.change || 4.20, 
        isUp: (quotes['SOLUSD']?.change || 0) >= 0 
      },
      'EUR/JPY': { 
        val: quotes['EURJPY']?.price || 162.05, 
        chg: quotes['EURJPY']?.change || 0.45, 
        isUp: (quotes['EURJPY']?.change || 0) >= 0 
      },
      'XAU/USD': { 
        val: quotes['XAUUSD']?.price || 2915.40, 
        chg: quotes['XAUUSD']?.change || 0.95, 
        isUp: (quotes['XAUUSD']?.change || 0) >= 0 
      },
      'GBP/USD': { 
        val: quotes['GBPUSD']?.price || 1.2590, 
        chg: quotes['GBPUSD']?.change || -0.06, 
        isUp: (quotes['GBPUSD']?.change || 0) >= 0 
      },
    };
  }, [quotes]);
  ...
  const assetsList = [
    { name: 'SOL/USD', desc: 'Solana' },
    { name: 'EUR/JPY', desc: 'Euro / Japanese Yen' },
    { name: 'BTC/USD', desc: 'Bitcoin' },
...
export default function HomeView({ quotes, setView, openSignUp, user }: HomeViewProps) {
  const { cmsContent } = useSiteCMS();
  // Real-time market quotes state for Popular Markets section
  const [liveQuotesMap, setLiveQuotesMap] = useState<Record<string, { price: number; change: number; prevPrice?: number }>>(() => {
    const initial: Record<string, { price: number; change: number; prevPrice?: number }> = {};
    Object.keys(quotes).forEach(sym => {
      initial[sym] = {
        price: quotes[sym].price,
        change: quotes[sym].change ?? 0,
        prevPrice: quotes[sym].price
      };
    });
    return initial;
  });

  // Keep synced with parent quotes updates
  useEffect(() => {
    setLiveQuotesMap(prev => {
      ...
      Object.keys(quotes).forEach(sym => {
        ...
        next[sym] = {
          price: quotes[sym].price,
          change: quotes[sym].change ?? 0,
          prevPrice: old ? old.price : quotes[sym].price
        };
      });
      return next;
    });
  ...
  return (
    <div className="flex flex-col bg-white">
      ...
      <div className="bg-white border-b border-slate-200 overflow-hidden py-2 flex items-center">
        <div className="whitespace-nowrap animate-[marquee_20s_linear_infinite] flex items-center gap-10 px-4">
          {[
            { sym: 'EURUSD', pair: 'EUR/USD', spread: '0.0', trend: 'up' },
            { sym: 'GBPUSD', pair: 'GBP/USD', spread: '0.1', trend: 'down' },
            { sym: 'XAUUSD', pair: 'XAU/USD', spread: '0.11', trend: 'up' },
            { sym: 'BTCUSD', pair: 'BTC/USD', spread: '15.0', trend: 'up' },
            { sym: 'USDJPY', pair: 'USD/JPY', spread: '0.1', trend: 'down' },
            { sym: 'AUDUSD', pair: 'AUD/USD', spread: '0.2', trend: 'up' },
            { sym: 'US30', pair: 'US30', spread: '1.0', trend: 'down' },
            { sym: 'UK100', pair: 'UK100', spread: '0.8', trend: 'up' },
          ].map((item, i) => (
            <div key={i} className="flex items-center gap-2">
              <AssetBrandLogo symbol={item.sym} size="sm" />
              ...
              <table className="w-full text-left text-xs font-bold text-slate-600">
                ...
                <tbody className="divide-y divide-slate-200">
                  {getFilteredInstruments().map((symbolObj) => {
                    ...
                    const fallbackQuote = DEFAULT_MARKET_QUOTES[symbolObj.symbol];
                    const basePrice = liveData?.price ?? fallbackQuote?.price ?? 1.0482;
                    const changeNum = liveData?.change ?? fallbackQuote?.change ?? 0;
