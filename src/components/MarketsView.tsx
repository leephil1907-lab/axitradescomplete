...
export default function MarketsView({
  ...
  const [marketFilter, setMarketFilter] = useState<'All' | 'Top Gainers' | 'Top Losers' | 'FX' | 'Crypto' | 'Commodities' | 'Indices' | 'Shares'>('All');
  const [showSessionsTracker, setShowSessionsTracker] = useState(false);
  ...
  const filteredMarkets = Object.values(quotes)
    .filter(q => {
      ...
      if (marketFilter === 'Commodities') return q.category === 'Commodities';
      if (marketFilter === 'Indices') return q.category === 'Indices';
      if (marketFilter === 'Shares') return q.category === 'Shares';
    ...
    <div className="bg-white min-h-screen pb-20 md:pb-10 font-sans text-slate-900 w-full">
      ...
      <div className="flex flex-col md:flex-row border-b border-slate-200 px-4 py-3 justify-between items-center bg-white sticky top-0 z-20 shadow-sm gap-3">
        <div className="flex items-center gap-3 w-full md:w-auto justify-between md:justify-start">
          ...
          {/* Session Mode Indicator & Global Sessions Toggle */}
          <div className="flex items-center gap-2">
            <button
              onClick={() => setShowSessionsTracker(!showSessionsTracker)}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-black transition border cursor-pointer ${
                showSessionsTracker 
                  ? 'bg-slate-900 text-brand-yellow border-slate-700 shadow-sm' 
            ...
            <div className="flex bg-slate-100 p-1 rounded-xl border border-slate-200">
              <div className="flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-black transition bg-red-600 text-white shadow-md">
      ...
      <div className="max-w-[1400px] mx-auto p-2 sm:p-4 flex flex-col gap-4">
        
        {/* Global Market Sessions Tracker Bar */}
        <AnimatePresence>
          {showSessionsTracker && (
            <motion.div
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden flex flex-col h-[400px] lg:h-[calc(100vh-200px)]">
            <div className="flex flex-col gap-2 px-4 py-3 border-b border-slate-100">
              <div className="flex items-center justify-between">
                <h3 className="font-bold text-slate-900">Markets ({filteredMarkets.length})</h3>
                <button onClick={() => setIsAlertsOpen(true)} className="flex items-center gap-1.5 bg-slate-100 hover:bg-slate-200 text-slate-700 px-2.5 py-1 rounded-lg text-xs font-bold transition cursor-pointer">
              ...
              <div className="flex items-center gap-1.5 overflow-x-auto hide-scrollbar text-[10px] font-bold py-1">
                {(['All', 'FX', 'Crypto', 'Commodities', 'Indices', 'Shares', 'Top Gainers', 'Top Losers'] as const).map(filter => (
                  <button 
                    key={filter} 
        ...
        <div className="lg:col-span-8 flex flex-col gap-4">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
            ...
            <div className="w-full">
              <TradingViewWidget symbol={selectedSymbol} theme="light" height={420} />
            </div>
          </div>
