...
export default function PlatformsView() {
  ...
  const baseRates: Record<string, number> = {
    'EURUSD': DEFAULT_MARKET_QUOTES['EURUSD']?.price || 1.0482,
    'GBPUSD': DEFAULT_MARKET_QUOTES['GBPUSD']?.price || 1.2590,
    'BTCUSD': DEFAULT_MARKET_QUOTES['BTCUSD']?.price || 96450.00,
    'XAUUSD': DEFAULT_MARKET_QUOTES['XAUUSD']?.price || 2915.40
  };

  useEffect(() => {
    setCurrentRate(baseRates[selectedSymbol]);
    // Initialize clean history for selected symbol
