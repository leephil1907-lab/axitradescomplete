...
import { getTawkToConfig, saveTawkToConfig, openTawkToChat, TawkToConfig } from '../utils/tawkto';
...
export default function AdminDashboardView({ 
  ...
  const [tawkConfig, setTawkConfig] = useState<TawkToConfig>(() => getTawkToConfig());
  const [tawkSnippetInput, setTawkSnippetInput] = useState<string>('');
  const [isTawkSaving, setIsTawkSaving] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    const handleTawkVisitorMsg = (e: any) => {
      const detail = e.detail;
      if (detail && detail.text) {
        showToast(`💬 Tawk.to Chat Message: "${detail.text.substring(0, 45)}${detail.text.length > 45 ? '...' : ''}"`, 'info');
      }
    };
  ...
  const handleRejectKYC = (docItem: any) => {
    ...
    fetch('/api/kyc/reject', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id: docItem.id || docItem.refCode, reason: 'Document verification failed or illegible' })
    }).catch(e => console.warn('Backend KYC reject sync:', e));
    ...
    try {
      ...
      const unsubscribe = onSnapshot(usersColRef, (snapshot) => {
        ...
        snapshot.forEach((docSnap) => {
          ...
          const name = data.displayName || data.name || (email ? email.split('@')[0].toUpperCase() : 'Trader');
          const kyc = data.kycStatus || 'NOT_STARTED';
          const status = data.status || (kyc === 'VERIFIED' ? 'Approved' : kyc === 'PENDING' ? 'Pending' : 'Approved');
          const liveBal = Number(data.liveBalance ?? data.balance ?? 0);
          const demoBal = Number(data.balance ?? 10000);
          const pnlPerc = Number(data.pnlPercentage ?? data.pnlOverride?.pnlPercentage ?? 24.5);
          ...
          firestoreUsers.push({
            ...
            pnlOverride: data.pnlOverride,
            registeredAt: data.createdAt ? new Date(data.createdAt).toISOString().replace('T', ' ').substring(0, 16) : new Date().toISOString().replace('T', ' ').substring(0, 16),
            lastActive: 'Active Now (' + new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) + ' UTC)',
            provider: data.providerId || 'Google / Email Auth',
            accountNo: data.accountNo || `AXI-MT5-${uid.substring(0, 6).toUpperCase()}`,
            accountType: data.accountType || 'Pro ECN Prime',
        ...
        if (firestoreUsers.length > 0) {
          setRecentUsers((prev) => {
            ...
            prev.forEach((pUser) => {
              if (!merged.some((m) => m.id === pUser.id || (pUser.email && m.email === pUser.email))) {
                merged.push(pUser);
              }
            });
            safeStorage.setItem('axi_registered_users', JSON.stringify(merged));
            return merged;
          });
        }
      }, (err) => {
        console.warn('Firestore live users listener notice:', err);
      });

      return () => unsubscribe();
    } catch (err) {
      console.warn('Could not initialize live users snapshot listener:', err);
    }
  }, []);
  ...
  useEffect(() => {
    const fetchBackendData = async () => {
      try {
        ...
        if (usersRes.ok) {
          ...
          if (uData.users && Array.isArray(uData.users) && uData.users.length > 0) {
            setRecentUsers(prev => {
              const serverUsers = uData.users;
              const merged = [...serverUsers];
              prev.forEach(p => {
                if (!merged.some(m => m.id === p.id || (p.email && m.email === p.email))) {
                  merged.push(p);
                }
              });
              return merged;
      ...
      try {
        ...
        if (kycRes.ok) {
          ...
          if (kData.documents && Array.isArray(kData.documents) && kData.documents.length > 0) {
            setKycDocs(prev => {
              const serverDocs = kData.documents;
              const merged = [...serverDocs];
              prev.forEach(p => {
                if (!merged.some(m => m.id === p.id || m.refCode === p.refCode)) {
                  merged.push(p);
                }
              });
              return merged;
      ...
      try {
        // 3. Fetch Partners, VPS, Promos
        const [pRes, vRes, prRes] = await Promise.all([
          fetch('/api/partners/list').catch(() => null),
          fetch('/api/vps/list').catch(() => null),
          fetch('/api/promos/list').catch(() => null)
        ]);
        if (pRes && pRes.ok) {
          const pData = await pRes.json();
          if (pData.applications) setPartnerApplications(pData.applications);
        }
        if (vRes && vRes.ok) {
          const vData = await vRes.json();
          if (vData.requests) setVpsRequests(vData.requests);
        }
        if (prRes && prRes.ok) {
          const prData = await prRes.json();
          if (prData.claims) setPromoClaims(prData.claims);
        }
      } catch (e) {}
    };
    ...
    const handleNewReg = () => {
      ...
      const savedDocs = safeStorage.getItem('axi_kyc_docs');
      if (savedDocs) {
        try { setKycDocs(JSON.parse(savedDocs)); } catch (e) {}
      }
      const savedPartners = safeStorage.getItem('axi_partner_applications');
      if (savedPartners) {
        try { setPartnerApplications(JSON.parse(savedPartners)); } catch (e) {}
      }
      const savedVps = safeStorage.getItem('axi_vps_requests');
      if (savedVps) {
        try { setVpsRequests(JSON.parse(savedVps)); } catch (e) {}
      }
      const savedPromos = safeStorage.getItem('axi_promo_claims');
      if (savedPromos) {
        try { setPromoClaims(JSON.parse(savedPromos)); } catch (e) {}
      }
    };

    window.addEventListener('axi_registered_user_event', handleNewReg);
    window.addEventListener('axi_user_update', handleNewReg);
    window.addEventListener('axi_kyc_update', handleNewReg);
    window.addEventListener('storage', handleNewReg);
    return () => {
      clearInterval(interval);
      window.removeEventListener('axi_registered_user_event', handleNewReg);
      window.removeEventListener('axi_user_update', handleNewReg);
      window.removeEventListener('axi_kyc_update', handleNewReg);
      window.removeEventListener('storage', handleNewReg);
    };
  }, []);
      ...
      .then(data => {
        if (data.success) {
          setSmtpConfig(prev => ({
            ...
            port: String(data.smtpPort || '465'),
            user: data.smtpUser || 'axicustomersupport@gmail.com',
            fromName: data.smtpFromName || 'Axi Trades Official',
            fromEmail: data.smtpFromEmail || 'axicustomersupport@gmail.com',
            isConfigured: data.isConfigured
          }));
          if (data.smtpUser) {
            setTestEmailRecipient(data.smtpUser);
          }
        }
      })
      .catch(e => console.warn('Could not load SMTP config:', e));
  }, []);
  ...
  const handleSaveSmtpConfig = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        showToast('✅ Google SMTP Server credentials saved successfully!', 'success');
        setSmtpConfig(prev => ({ ...prev, isConfigured: data.config.isConfigured }));
      } else {
  ...
  const handleSendTestEmail = async () => {
    if (!testEmailRecipient) {
      showToast('Please enter a target recipient email for the test', 'error');
      return;
    }
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        setSmtpStatusMessage(`✅ Direct delivery confirmed to ${testEmailRecipient} (Provider: ${data.provider || 'Google SMTP'})`);
        showToast(`✉️ Live Test Email delivered to ${testEmailRecipient}! Check your inbox.`, 'success');
      } else {
        setSmtpStatusMessage(`⚠️ SMTP Delivery Notice: ${data.error || data.notice}`);
        showToast(`SMTP Notice: ${data.error || 'Check Google SMTP credentials'}`, 'info');
      }
    } catch (err: any) {
      setSmtpStatusMessage(`❌ Test failed: ${err.message}`);
  ...
  const handleQuickAdjustUserBalance = (userId: string, amount: number, customReason: string = 'Admin Balance Top-up / Deduction') => {
    ...
    fetch(`/api/users/${userId}/balance`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ balance: calculatedNewBal, reason: customReason })
    }).catch(e => console.warn('Backend user balance update sync:', e));
    ...
    if (targetEmail) {
      ...
      try {
        ...
        setDoc(userDocRef, {
          liveBalance: Math.max(0, (recentUsers.find(u => u.id === userId)?.balance || 0) + amount),
          updatedAt: Date.now()
        }, { merge: true }).catch(() => {});
      } catch (e) {}
      ...
      const payload = {
        ...
        type: amount >= 0 ? 'DepositApproved' : 'Withdrawal',
        amount: Math.abs(amount),
        status: 'Approved',
        timestamp: new Date().toISOString().replace('T', ' ').substring(0, 16) + ' UTC',
        method: 'Administrative Portfolio Adjustment',
        reason: customReason
      };
      window.dispatchEvent(new CustomEvent('axi_email_trigger', { detail: payload }));
    }
  ...
  const handleSetExactUserBalance = (userId: string, targetBalance: number, reason: string = 'Admin Exact Balance Override') => {
    ...
    fetch(`/api/users/${userId}/balance`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ balance: Math.max(0, targetBalance), reason })
    }).catch(e => console.warn('Backend user balance exact override sync:', e));
    ...
    if (targetEmail) {
      ...
      try {
        const userDocRef = doc(db, 'users', userId);
        setDoc(userDocRef, {
          liveBalance: Math.max(0, targetBalance),
          updatedAt: Date.now()
        }, { merge: true }).catch(() => {});
      } catch (e) {}
  ...
  const handleCreateNewClient = (e: React.FormEvent) => {
    ...
    }).catch(e => console.warn('Backend client onboarding sync:', e));
  ...
  const handleUpdateUserStatus = (userId: string, newStatus: 'Pending' | 'Approved' | 'Flagged', customNote?: string) => {
    ...
    fetch(`/api/users/${userId}/status`, {
      ...
      body: JSON.stringify({
        status: newStatus,
        verificationStatus: newStatus,
        kycStatus: newStatus === 'Approved' ? 'VERIFIED' : newStatus === 'Flagged' ? 'REJECTED' : 'PENDING'
      })
    }).catch(e => console.warn('Backend user status sync notice:', e));
  ...
  const handleApproveDeposit = (dep: any) => {
    ...
    const payload = {
      ...
      type: dep.type === 'Withdrawal' ? 'Withdrawal' : 'DepositApproved',
      amount: dep.amount || 0,
      status: 'Approved',
      timestamp: new Date().toISOString().replace('T', ' ').substring(0, 16) + ' UTC',
  ...
  const handleRejectDeposit = (dep: any) => {
    ...
    const payload = {
      ...
      txId: dep.id,
      txType: dep.type || 'Deposit',
      type: dep.type === 'Withdrawal' ? 'Withdrawal' : 'Transaction',
      amount: dep.amount || 0,
      status: 'Rejected',
      timestamp: new Date().toISOString().replace('T', ' ').substring(0, 16) + ' UTC',
      method: dep.method || 'Bank Wire / Card',
      ...
      <div className={`${isSidebarCollapsed ? 'w-16' : 'w-64'} bg-[#222d32] text-white flex flex-col shrink-0 overflow-y-auto transition-all duration-200`}>
        ...
        <div className="flex-1 py-2 flex flex-col">
          {[
            ...
            { id: 'partnerApplications', label: 'Partner & IB Enquiries', icon: Users, badge: partnerApplications.filter(p => p.status === 'Pending Review').length },
            { id: 'vpsRequests', label: 'Forex VPS Requests', icon: Server, badge: vpsRequests.filter(v => v.status === 'Pending Provisioning').length },
            { id: 'promoClaims', label: 'Promotions & Bonus Claims', icon: Award, badge: promoClaims.filter(c => c.status === 'Pending Approval').length },
            { id: 'tawktoSettings', label: 'Tawk.to Live Chat Desk', icon: MessageSquare },
              ...
              >
                ...
                {!isSidebarCollapsed && (
                  <div className="flex items-center gap-1.5">
                    {item.badge ? (
                      <span className="bg-[#FF9800] text-black text-[10px] font-black px-1.5 py-0.5 rounded-full">
                        {item.badge}
                      </span>
                    ) : null}
        ...
        <main className="flex-1 overflow-y-auto p-4 sm:p-6 bg-[#ecf0f5]">
          ...
          {activeTab === 'liveChat' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-6">
              ...
              {/* Tawk.to Live Chat Configuration & Integration Panel */}
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl text-white">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-base font-black text-white flex items-center gap-2">
                        Tawk.to Official Live Chat Integration
                        <span className={`text-[10px] px-2.5 py-0.5 rounded-full font-black uppercase tracking-wider border ${
                          tawkConfig.enabled && tawkConfig.propertyId 
                            ? 'bg-emerald-500/20 text-emerald-400 border-emerald-500/30' 
                            : 'bg-amber-500/20 text-amber-300 border-amber-500/30'
                        }`}>
                          {tawkConfig.enabled && tawkConfig.propertyId ? '● Live & Connected' : '○ Standby / Custom ID'}
                        </span>
                      </h3>
                      <p className="text-xs text-slate-400 mt-0.5">
                        Embed your Tawk.to Property ID and Widget ID to chat directly with traders from your tawk.to dashboard or mobile app.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => {
                        ...
                        if (opened) {
                          showToast('💬 Tawk.to Live Chat opened for testing.', 'success');
                        } else {
                          showToast('Tawk.to widget is loading... If blocked, verify Property ID.', 'info');
                        }
                      }}
                      className="bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold px-3.5 py-2 rounded-xl transition flex items-center gap-1.5 cursor-pointer shadow-md"
                    >
                      <ExternalLink className="w-3.5 h-3.5" /> Test Open Widget
                    </button>
                    <a
                      href="https://dashboard.tawk.to"
                      target="_blank"
                      rel="noopener noreferrer"
                      className="bg-slate-800 hover:bg-slate-700 text-slate-300 border border-slate-700 text-xs font-bold px-3.5 py-2 rounded-xl transition flex items-center gap-1.5 cursor-pointer"
                    >
                      <ExternalLink className="w-3.5 h-3.5" /> Tawk.to Agent Login
                    </a>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="text-[11px] font-black uppercase text-slate-400 mb-1.5 block">
                      Tawk.to Property ID
                    </label>
                    <input
                      type="text"
                      placeholder="e.g. 64a8b72c94cf5d49dc62a123"
                      value={tawkConfig.propertyId}
                      onChange={(e) => setTawkConfig(prev => ({ ...prev, propertyId: e.target.value.trim() }))}
                  ...
                  <div>
                    ...
                    <input
                      type="text"
                      placeholder="e.g. 1h3k7p9q1 or default"
                      value={tawkConfig.widgetId}
                      onChange={(e) => setTawkConfig(prev => ({ ...prev, widgetId: e.target.value.trim() }))}
                      className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-emerald-500 font-mono"
                    />
                    <span className="text-[10px] text-slate-500 mt-1 block">
                      Default is usually <code className="text-emerald-400">default</code> or your custom widget ID.
                    </span>
                  </div>
                </div>

                {/* Instructions for AXI logo avatar and direct push notifications */}
                <div className="mt-4 p-4 rounded-xl bg-slate-950/70 border border-slate-800 text-xs text-slate-300 space-y-2.5">
                  <div className="font-bold text-white flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-brand-red"></span>
                    <span>Direct Notifications & Axi Logo Avatar Customization</span>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-[11px]">
                    <div className="bg-slate-900/90 p-3 rounded-lg border border-slate-800">
                      <div className="font-bold text-emerald-400 mb-1">1. Update Chat Bot Avatar to Axi Logo</div>
                      <p className="text-slate-400">
                        In your <strong>Tawk.to Dashboard</strong> → <strong>Administration</strong> → <strong>Chat Widget</strong> → <strong>Widget Appearance</strong> → Upload your Axi Logo (<code className="text-brand-red">https://www.axitrades.com/axi/axi-logo.svg</code> or red AXI badge).
                      </p>
                    </div>
                    <div className="bg-slate-900/90 p-3 rounded-lg border border-slate-800">
                      <div className="font-bold text-emerald-400 mb-1">2. Direct Mobile & Desktop Push Notifications</div>
                      <p className="text-slate-400">
                        Download the <strong>Tawk.to Mobile App</strong> (iOS/Android) or keep the Tawk.to Agent Console open. Whenever a visitor sends a message, you will receive instant push and audio alerts directly on your device.
                      </p>
                    </div>
                  </div>
                </div>
                <div className="mt-4 pt-4 border-t border-slate-800">
                  <label className="text-[11px] font-black uppercase text-slate-400 mb-1.5 block flex items-center justify-between">
                    <span>Or Paste Full Tawk.to Embed Script / URL</span>
                    <span className="text-[10px] text-emerald-400 lowercase font-normal">Auto-extracts Property & Widget IDs</span>
                  </label>
                  <div className="flex gap-2">
                    <input
                      type="text"
                      placeholder='e.g. https://embed.tawk.to/64a8b72c94cf5d49dc62a123/1h3k7p9q1 or full <script> tag'
                      value={tawkSnippetInput}
                      onChange={(e) => {
                        const val = e.target.value;
                        setTawkSnippetInput(val);
                        const match = val.match(/embed\.tawk\.to\/([a-zA-Z0-9_-]+)\/([a-zA-Z0-9_-]+)/);
                        if (match) {
                          setTawkConfig(prev => ({
                            ...prev,
                            propertyId: match[1],
                          ...
                          showToast('✨ Extracted Tawk.to Property and Widget IDs!', 'success');
                        }
                      }}
                      className="flex-1 bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-emerald-500 font-mono"
                    />
                    <button
                      type="button"
                      onClick={() => {
                        setIsTawkSaving(true);
                        saveTawkToConfig(tawkConfig);
                        setTimeout(() => {
                          setIsTawkSaving(false);
                          showToast('💾 Tawk.to configuration saved & loaded across the website!', 'success');
                        }, 400);
                      }}
                      disabled={isTawkSaving}
                    ...
                    >
                      ...
                      {isTawkSaving ? 'Saving...' : 'Save Tawk.to Settings'}
                    </button>
                  </div>
                </div>
              </div>
          ...
          {activeTab === 'sendEmail' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-8">
              <div className="border-b border-slate-200 pb-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                  <h2 className="text-xl font-black text-slate-900 flex items-center gap-2">
                    <Mail className="w-6 h-6 text-brand-red" /> Google SMTP & Email Delivery Management
                  </h2>
                  <p className="text-xs text-slate-500 mt-1">Configure live Google SMTP data, test inbox arrival, and dispatch transactional notifications directly to registered users.</p>
                </div>
                <div className="flex items-center gap-2">
                  <span className={`text-xs font-black uppercase px-3 py-1.5 rounded-full border flex items-center gap-1.5 ${
                    smtpConfig.isConfigured 
                      ? 'bg-emerald-50 text-emerald-700 border-emerald-300' 
                      : 'bg-blue-50 text-blue-700 border-blue-200'
                  }`}>
                    <span className="w-2 h-2 rounded-full bg-current animate-pulse"></span>
                    Google SMTP: {smtpConfig.host}:{smtpConfig.port}
                  </span>
                </div>
              </div>

              {/* 2-Column Grid: Google SMTP Server Config + Instant Test Mailer */}
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                
                {/* Panel 1: Google SMTP Configuration */}
                <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between pb-3 mb-4 border-b border-slate-100">
                      <div className="flex items-center gap-2">
                        <div className="w-8 h-8 rounded-lg bg-red-50 text-brand-red flex items-center justify-center font-black">
                          G
                        </div>
                        <div>
                          <h3 className="font-bold text-slate-900 text-sm">Google Email SMTP Server</h3>
                          <p className="text-[11px] text-slate-500">Official Gmail / Google Workspace Transport</p>
                        </div>
                      </div>
                    ...
                    <form onSubmit={handleSaveSmtpConfig} className="space-y-4">
                      <div className="grid grid-cols-3 gap-3">
                        <div className="col-span-2">
                          <label className="text-[11px] font-extrabold uppercase tracking-wider text-slate-600 block mb-1">Google SMTP Host</label>
                          <input 
                            type="text" 
                            value={smtpConfig.host}
                            onChange={e => setSmtpConfig({ ...smtpConfig, host: e.target.value })}
                            className="w-full border border-slate-200 rounded-xl px-3 py-2 text-xs font-mono bg-slate-50 outline-none focus:border-brand-red focus:bg-white"
                            placeholder="smtp.gmail.com"
                            required
                          />
                        </div>
                        <div>
                          <label className="text-[11px] font-extrabold uppercase tracking-wider text-slate-600 block mb-1">Port</label>
                          <select 
                      ...
                      <div>
                        <label className="text-[11px] font-extrabold uppercase tracking-wider text-slate-600 block mb-1">Google Account Email (SMTP User)</label>
                        <input 
                          type="email" 
                          value={smtpConfig.user}
                          onChange={e => setSmtpConfig({ ...smtpConfig, user: e.target.value, fromEmail: e.target.value })}
                          className="w-full border border-slate-200 rounded-xl px-3 py-2 text-xs font-medium bg-slate-50 outline-none focus:border-brand-red focus:bg-white"
                          placeholder="axicustomersupport@gmail.com"
                          required
                        />
                      </div>
                      ...
                      <div>
                        <label className="text-[11px] font-extrabold uppercase tracking-wider text-slate-600 block mb-1">
                          Google App Password
                          <span className="text-[10px] font-normal text-slate-400 lowercase ml-1">(from Google Security &gt; 2-Step Verification &gt; App Passwords)</span>
                        </label>
                        <input 
                          type="password" 
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="text-[11px] font-extrabold uppercase tracking-wider text-slate-600 block mb-1">Sender Brand Name</label>
                          <input 
                            type="text" 
                            value={smtpConfig.fromName}
                            onChange={e => setSmtpConfig({ ...smtpConfig, fromName: e.target.value })}
                            className="w-full border border-slate-200 rounded-xl px-3 py-2 text-xs bg-slate-50 outline-none focus:border-brand-red focus:bg-white"
                            placeholder="Axi Trades Official"
                          />
                        </div>
                        <div>
                          <label className="text-[11px] font-extrabold uppercase tracking-wider text-slate-600 block mb-1">Sender From Address</label>
                          <input 
                            type="email" 
                      ...
                      <button
                        ...
                        className="w-full bg-slate-900 hover:bg-black text-white font-bold py-2.5 rounded-xl text-xs flex items-center justify-center gap-2 transition cursor-pointer mt-2"
                      >
                        {isSavingSmtp ? <RefreshCw className="w-4 h-4 animate-spin" /> : <ShieldCheck className="w-4 h-4 text-emerald-400" />}
                        Save Google SMTP Configuration
                      </button>
                    </form>
                  </div>

                  <div className="mt-4 bg-slate-50 border border-slate-200 rounded-xl p-3 text-[11px] text-slate-600 leading-relaxed">
                    💡 <strong>Google SMTP Note:</strong> Standard Google Mail accounts require generating a 16-character <em>App Password</em> in your Google Account security settings with 2-Step Verification enabled.
                  </div>
                </div>

                {/* Panel 2: Live Test Delivery Console */}
                <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm flex flex-col justify-between">
                  <div>
                    <div className="flex items-center justify-between pb-3 mb-4 border-b border-slate-100">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <h3 className="font-bold text-slate-900 text-sm">Direct Live SMTP Inbox Test</h3>
                          <p className="text-[11px] text-slate-500">Send instant test email directly to any registered email</p>
                        </div>
                      </div>
                    ...
                    <div className="space-y-4">
                      <div>
                        <label className="text-[11px] font-extrabold uppercase tracking-wider text-slate-600 block mb-1">Target Test Email Address</label>
                        <input 
                          type="email" 
                          value={testEmailRecipient}
                          onChange={e => setTestEmailRecipient(e.target.value)}
                          className="w-full border border-slate-200 rounded-xl px-3 py-2 text-xs bg-slate-50 outline-none focus:border-blue-500 focus:bg-white font-medium"
                          placeholder="e.g. yourname@gmail.com"
                        />
                      </div>

                      <div>
                        <label className="text-[11px] font-extrabold uppercase tracking-wider text-slate-600 block mb-1">Axi Template To Test</label>
                        <select 
                          value={testEmailTemplate}
                          onChange={e => setTestEmailTemplate(e.target.value)}
                          className="w-full border border-slate-200 rounded-xl px-3 py-2 text-xs bg-slate-50 outline-none focus:border-blue-500 focus:bg-white font-medium"
                        >
                          <option value="Registration">🎉 Welcome & Account Initialized Template</option>
                          <option value="DepositApproved">💳 Deposit Approved & Balance Credited Template</option>
                          <option value="Withdrawal">💸 Withdrawal Dispatched & Payout Template</option>
                          <option value="KYC">🛡️ KYC Identity Verification Approved Template</option>
                          <option value="PasswordReset">🔑 6-Digit Password Reset Code Template</option>
                        </select>
                      </div>
                      ...
                      <button
                        ...
                        className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 rounded-xl text-xs flex items-center justify-center gap-2 shadow-sm transition cursor-pointer"
                      >
                        {isSendingTest ? (
                          <>
                            <RefreshCw className="w-4 h-4 animate-spin" />
                      ...
                      {smtpStatusMessage && (
                        <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs font-medium text-slate-700">
                          {smtpStatusMessage}
                        </div>
                      )}
                    </div>
                  ...
                  <div className="mt-4 bg-emerald-50 border border-emerald-200 rounded-xl p-3 text-[11px] text-emerald-800 leading-relaxed">
                    ✨ <strong>Direct Delivery:</strong> Emails dispatched through this engine bypass all simulated popups and deliver directly into the user's real email inbox with Axi corporate responsive HTML formatting.
                  </div>
                </div>

              </div>

              {/* Panel 3: Direct Trader Notice & Broadcast Dispatcher */}
              <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm">
                <div className="flex items-center justify-between pb-3 mb-5 border-b border-slate-100">
                  <div>
                    <h3 className="font-bold text-slate-900 text-base flex items-center gap-2">
                      <MessageSquare className="w-5 h-5 text-brand-red" />
                      Axi Trader Email Dispatcher
                    </h3>
                    <p className="text-xs text-slate-500">Dispatch bespoke administrative emails directly to user registered inboxes.</p>
                  </div>
                </div>
                ...
                <div className="space-y-4 max-w-4xl">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      <label className="text-[11px] font-extrabold uppercase tracking-wider text-slate-600 block mb-1">Target Trader / Audience</label>
                      <select 
                        value={emailRecipient} 
                        onChange={e => setEmailRecipient(e.target.value)}
                        className="w-full border border-slate-200 rounded-xl p-2.5 text-xs bg-slate-50 outline-none focus:border-brand-red focus:bg-white font-medium"
                      >
                    ...
                    <div>
                      <label className="text-[11px] font-extrabold uppercase tracking-wider text-slate-600 block mb-1">Email Notice Template</label>
                      <select 
                        value={emailType} 
                      ...
                      >
                        ...
                        <option value="Registration">Welcome Registration Template</option>
                        <option value="PasswordReset">Password Reset Verification</option>
                        <option value="DepositApproved">Deposit Approval Notice</option>
                        <option value="Withdrawal">Withdrawal Clearance Notice</option>
                        <option value="KYC">KYC Verification Approval</option>
                      </select>
                    </div>
                  </div>

                  <div>
                    <label className="text-[11px] font-extrabold uppercase tracking-wider text-slate-600 block mb-1">Subject Line</label>
                    <input 
                      type="text" 
                      value={emailSubject} 
                  ...
                  <div>
                    <label className="text-[11px] font-extrabold uppercase tracking-wider text-slate-600 block mb-1">Message Body</label>
                    <textarea 
                      value={emailBody} 
                      onChange={e => setEmailBody(e.target.value)}
                      rows={5}
                      className="w-full border border-slate-200 rounded-xl p-3 text-xs bg-slate-50 outline-none focus:border-brand-red focus:bg-white font-sans leading-relaxed"
                    />
                  </div>
                  ...
                  <button 
                    onClick={() => {
                      ...
                      window.dispatchEvent(new CustomEvent('axi_email_trigger', { detail: payload }));
                      showToast(`✉️ Email dispatched to ${emailRecipient} via Google SMTP!`, 'success');
                    }}
                    className="bg-brand-red hover:bg-red-700 text-white font-extrabold px-6 py-3 rounded-xl flex items-center gap-2 shadow-md transition cursor-pointer text-xs"
                  >
                    <Send className="w-4 h-4" /> Dispatch Official Axi Email to Registered Trader
                  </button>
                </div>
              </div>
          ...
          {/* Partner & IB Applications Management */}
          {activeTab === 'partnerApplications' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-6">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200 pb-4">
                <div>
                  <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                    <Users className="w-5 h-5 text-[#FF9800]" /> Introducing Broker (IB) & Affiliate Applications
                  </h2>
                  <p className="text-xs text-slate-500 mt-1">Review, approve, and provision bespoke rebate tiers for registered IB partners, fund managers, and affiliates.</p>
                </div>
                <span className="text-xs font-bold bg-amber-50 text-amber-800 border border-amber-300 px-3 py-1.5 rounded-full">
                  {partnerApplications.filter(p => p.status === 'Pending Review').length} Pending Enquiries
                </span>
              </div>

              {partnerApplications.length === 0 ? (
                <div className="text-center py-12 bg-white border border-dashed border-slate-200 rounded-2xl p-8">
                  <CheckCircle2 className="w-10 h-10 text-emerald-500 mx-auto mb-2" />
                  <p className="text-slate-700 font-bold">No partner applications submitted yet.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 gap-4">
                  {partnerApplications.map((partner, idx) => (
                    <div key={partner.appId || idx} className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                      <div className="flex items-start gap-4">
                        <div className="w-12 h-12 rounded-xl bg-amber-50 text-amber-700 flex items-center justify-center font-black text-sm shrink-0 border border-amber-200">
                          {partner.partnerType || 'IB'}
                        </div>
                        <div className="space-y-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="font-bold text-slate-900 text-base">{partner.fullName}</span>
                            <span className="text-xs font-mono text-slate-500">({partner.email})</span>
                            <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full ${
                              partner.status === 'Approved' ? 'bg-emerald-100 text-emerald-800 border border-emerald-300' :
                              partner.status === 'Declined' ? 'bg-rose-100 text-rose-800 border border-rose-300' :
                              'bg-amber-100 text-amber-900 border border-amber-300 animate-pulse'
                            }`}>
                              {partner.status}
                            </span>
                          </div>
                          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 text-xs text-slate-600 font-medium mt-2">
                            <div><span className="text-slate-400">Phone:</span> {partner.phone || 'N/A'}</div>
                            <div><span className="text-slate-400">Telegram:</span> <strong className="text-blue-600">{partner.telegramHandle || 'N/A'}</strong></div>
                            <div><span className="text-slate-400">Est. Volume:</span> <strong className="text-slate-900">{partner.estimatedMonthlyLots || 'N/A'}</strong></div>
                            <div><span className="text-slate-400">Country:</span> {partner.country || 'Global'}</div>
                          </div>
                          {partner.marketingChannels && (
                            <p className="text-xs text-slate-500 bg-slate-50 p-2 rounded-lg border border-slate-100 mt-2">
                              <strong>Channels:</strong> {partner.marketingChannels}
                            </p>
                          )}
                        </div>
                      </div>

                      <div className="flex items-center gap-2 w-full lg:w-auto shrink-0 border-t lg:border-t-0 pt-3 lg:pt-0 border-slate-100">
                        {partner.status === 'Pending Review' ? (
                          <>
                            <button
                              onClick={() => {
                                setPartnerApplications(prev => prev.map(p => (p.appId === partner.appId || p.email === partner.email) ? { ...p, status: 'Approved' } : p));
                                fetch('/api/email/send', {
                                  ...
                                  body: JSON.stringify({
                                    ...
                                    type: 'Custom',
                                    subject: `🤝 Welcome to Axi Institutional Partner Program [Ref: ${partner.appId}]`,
                                    customBody: `Dear ${partner.fullName},\n\nWe are pleased to notify you that your application for the Axi ${partner.partnerType || 'Introducing Broker'} Program has been APPROVED.\n\nYour institutional multi-tier rebate portal and master affiliate referral links have been initialized. You earn competitive commissions per lot with instant daily rebates.\n\nBest regards,\nAxi Institutional Partnerships Team\nwww.axi.com`
                                  })
                                }).catch(() => {});
                                showToast(`✅ Partner application approved for ${partner.fullName}!`, 'success');
                              }}
                            ...
                            <button
                              onClick={() => {
                                setPartnerApplications(prev => prev.map(p => (p.appId === partner.appId || p.email === partner.email) ? { ...p, status: 'Declined' } : p));
                                showToast(`Partner application declined.`, 'info');
                              }}
                          ...
                          <span className="text-xs font-bold text-slate-500 bg-slate-100 px-3 py-1.5 rounded-lg">
                            Decision Recorded ({partner.status})
                          </span>
                        )}
                      </div>
                    </div>
          ...
          {/* Forex VPS Provisioning Requests */}
          {activeTab === 'vpsRequests' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-6">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200 pb-4">
                <div>
                  <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                    <Server className="w-5 h-5 text-indigo-600" /> Forex VPS Server Provisioning Desk
                  </h2>
                  <p className="text-xs text-slate-500 mt-1">Manage, approve, and provision low-latency MT4/MT5 Equinix VPS instances for automated EA traders.</p>
                </div>
                <span className="text-xs font-bold bg-indigo-50 text-indigo-800 border border-indigo-200 px-3 py-1.5 rounded-full">
                  {vpsRequests.filter(v => v.status === 'Pending Provisioning').length} Pending VPS Requests
                </span>
              </div>
              ...
              {vpsRequests.length === 0 ? (
                <div className="text-center py-12 bg-white border border-dashed border-slate-200 rounded-2xl p-8">
                  ...
                  <p className="text-slate-700 font-bold">No VPS requests pending.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 gap-4">
                  {vpsRequests.map((vps, idx) => (
                    <div key={vps.vpsRef || idx} className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                      <div className="flex items-start gap-4">
                        <div className="w-12 h-12 rounded-xl bg-indigo-50 text-indigo-700 flex items-center justify-center font-black shrink-0 border border-indigo-200">
                          <Server className="w-6 h-6" />
                        </div>
                        <div className="space-y-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="font-bold text-slate-900 text-base">{vps.clientName}</span>
                            <span className="text-xs font-mono text-slate-500">({vps.clientEmail})</span>
                            <span className="text-xs font-mono font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded border border-indigo-100">
                              Ref: {vps.vpsRef}
                            </span>
                            <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full ${
                              vps.status === 'Provisioned' ? 'bg-emerald-100 text-emerald-800 border border-emerald-300' :
                              vps.status === 'Rejected' ? 'bg-rose-100 text-rose-800 border border-rose-300' :
                              'bg-amber-100 text-amber-900 border border-amber-300 animate-pulse'
                            }`}>
                              {vps.status}
                            </span>
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 text-xs text-slate-600 font-medium mt-2">
                            <div><span className="text-slate-400">Tier:</span> <strong className="text-slate-800">{vps.selectedTier}</strong></div>
                            <div><span className="text-slate-400">Location:</span> <strong className="text-slate-800">{vps.serverLocation}</strong></div>
                            <div><span className="text-slate-400">Platform:</span> <strong className="text-slate-800">{vps.platform}</strong></div>
                            <div><span className="text-slate-400">Account #:</span> <strong className="text-slate-800 font-mono">{vps.accountNumber}</strong></div>
                          </div>
                          {vps.eaStrategyName && (
                            <p className="text-xs text-slate-500 bg-slate-50 p-2 rounded-lg border border-slate-100 mt-2">
                              <strong>EA / Strategy:</strong> {vps.eaStrategyName}
                            </p>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2 w-full lg:w-auto shrink-0 border-t lg:border-t-0 pt-3 lg:pt-0 border-slate-100">
                        {vps.status === 'Pending Provisioning' ? (
                          <>
                            <button
                              onClick={() => {
                                setVpsRequests(prev => prev.map(v => (v.vpsRef === vps.vpsRef || v.clientEmail === vps.clientEmail) ? { ...v, status: 'Provisioned' } : v));
                                fetch('/api/email/send', {
                                  method: 'POST',
                                  headers: { 'Content-Type': 'application/json' },
                                  body: JSON.stringify({
                                    recipientEmail: vps.clientEmail,
                                    recipientName: vps.clientName,
                                    type: 'Custom',
                                    subject: `⚡ Axi Forex VPS Provisioned & Ready [Ref: ${vps.vpsRef}]`,
                                    customBody: `Dear ${vps.clientName},\n\nYour Axi ${vps.selectedTier} hosted at ${vps.serverLocation} has been PROVISIONED.\n\nRDP IP: 185.193.140.${Math.floor(10 + Math.random() * 80)}\nPort: 3389\nUsername: Administrator\nTemporary Password: Axi-VPS-${Math.floor(10000 + Math.random() * 90000)}!\n\nYour MT4/MT5 trading terminal has been pre-configured with direct optical connection (<1ms) to Axi liquidity bridges.\n\nBest regards,\nAxi Server Engineering Desk\nwww.axi.com`
                                  })
                                }).catch(() => {});
                                showToast(`✅ VPS Provisioned and credentials emailed to ${vps.clientName}!`, 'success');
                              }}
                            ...
                            >
                              <CheckCircle2 className="w-4 h-4" /> Provision VPS
                            </button>
                            <button
                              onClick={() => {
                                setVpsRequests(prev => prev.map(v => (v.vpsRef === vps.vpsRef || v.clientEmail === vps.clientEmail) ? { ...v, status: 'Rejected' } : v));
                                showToast(`VPS request rejected.`, 'info');
                              }}
                              className="bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs px-4 py-2 rounded-xl transition cursor-pointer flex items-center gap-1.5"
                            >
                              <XCircle className="w-4 h-4" /> Reject
          ...
          {/* Promotions & Bonus Claims */}
          {activeTab === 'promoClaims' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-6">
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200 pb-4">
                <div>
                  <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                    <Award className="w-5 h-5 text-amber-500" /> Promotions & Deposit Bonus Claims
                  </h2>
                  <p className="text-xs text-slate-500 mt-1">Review trader promotion claims and instantly credit trading bonuses directly to trader accounts with automated confirmation emails.</p>
                </div>
                <span className="text-xs font-bold bg-amber-50 text-amber-800 border border-amber-200 px-3 py-1.5 rounded-full">
                  {promoClaims.filter(c => c.status === 'Pending Approval').length} Pending Bonus Claims
                </span>
              </div>
              ...
              {promoClaims.length === 0 ? (
                <div className="text-center py-12 bg-white border border-dashed border-slate-200 rounded-2xl p-8">
                  ...
                  <p className="text-slate-700 font-bold">No promotion claims currently pending.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 gap-4">
                  {promoClaims.map((claim, idx) => (
                    <div key={claim.claimId || idx} className="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                      <div className="flex items-start gap-4">
                        <div className="w-12 h-12 rounded-xl bg-amber-50 text-amber-600 flex items-center justify-center font-black shrink-0 border border-amber-200">
                          <Award className="w-6 h-6" />
                        </div>
                        <div className="space-y-1">
                          <div className="flex items-center gap-2 flex-wrap">
                            <span className="font-bold text-slate-900 text-base">{claim.clientName}</span>
                            <span className="text-xs font-mono text-slate-500">({claim.clientEmail})</span>
                            <span className="text-xs font-mono font-bold text-amber-700 bg-amber-50 px-2 py-0.5 rounded border border-amber-100">
                              Claim: {claim.claimId}
                            </span>
                            <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full ${
                              claim.status === 'Credited' ? 'bg-emerald-100 text-emerald-800 border border-emerald-300' :
                              claim.status === 'Declined' ? 'bg-rose-100 text-rose-800 border border-rose-300' :
                              'bg-amber-100 text-amber-900 border border-amber-300 animate-pulse'
                            }`}>
                              {claim.status}
                            </span>
                          </div>
                          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-xs text-slate-600 font-medium mt-2">
                            <div><span className="text-slate-400">Promo:</span> <strong className="text-slate-800">{claim.promoTitle}</strong></div>
                            <div><span className="text-slate-400">Bonus Value:</span> <strong className="text-emerald-600 font-mono font-bold">+${(claim.bonusAmount || 1000).toLocaleString()} USD</strong></div>
                            <div><span className="text-slate-400">Account #:</span> <strong className="text-slate-800 font-mono">{claim.accountNumber}</strong></div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-2 w-full lg:w-auto shrink-0 border-t lg:border-t-0 pt-3 lg:pt-0 border-slate-100">
                        {claim.status === 'Pending Approval' ? (
                          <>
                            <button
                              onClick={() => {
                                const bonusVal = Number(claim.bonusAmount) || 1000;
                                setPromoClaims(prev => prev.map(c => (c.claimId === claim.claimId || c.clientEmail === claim.clientEmail) ? { ...c, status: 'Credited' } : c));
                                handleQuickAdjustUserBalance(claim.clientEmail, bonusVal, `Promo Bonus: ${claim.promoTitle}`);
                                fetch('/api/email/send', {
                                  method: 'POST',
                                  headers: { 'Content-Type': 'application/json' },
                                  body: JSON.stringify({
                                    recipientEmail: claim.clientEmail,
                                    recipientName: claim.clientName,
                                    type: 'Custom',
                                    subject: `🎁 Promotion Bonus Credited: +$${bonusVal.toLocaleString()} USD [Ref: ${claim.claimId}]`,
                                    customBody: `Dear ${claim.clientName},\n\nCongratulations! Your promotion claim for "${claim.promoTitle}" has been APPROVED.\n\nA trading bonus of +$${bonusVal.toLocaleString()} USD has been credited directly to your live trading account (${claim.accountNumber}).\n\nHappy trading!\n\nBest regards,\nAxi Promotions Desk\nwww.axi.com`
                                  })
                                }).catch(() => {});
                                showToast(`🎁 Bonus +$${bonusVal.toLocaleString()} credited and emailed to ${claim.clientName}!`, 'success');
                              }}
                              className="bg-amber-500 hover:bg-amber-600 text-slate-950 font-black text-xs px-4 py-2 rounded-xl transition cursor-pointer flex items-center gap-1.5 shadow-xs"
                            >
                              <CheckCircle2 className="w-4 h-4" /> Credit Bonus (+${(claim.bonusAmount || 1000).toLocaleString()})
                            </button>
                            <button
                              onClick={() => {
                                setPromoClaims(prev => prev.map(c => (c.claimId === claim.claimId || c.clientEmail === claim.clientEmail) ? { ...c, status: 'Declined' } : c));
                                showToast(`Promotion claim declined.`, 'info');
                              }}
                              className="bg-rose-600 hover:bg-rose-700 text-white font-bold text-xs px-4 py-2 rounded-xl transition cursor-pointer flex items-center gap-1.5"
                            >
                              <XCircle className="w-4 h-4" /> Decline
          ...
          {/* Tawk.to Live Chat Support Integration Desk */}
          {activeTab === 'tawktoSettings' && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col gap-6">
              {/* Header */}
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-200 pb-4">
                <div>
                  <h2 className="text-xl font-bold text-slate-900 flex items-center gap-2">
                    <MessageSquare className="w-5 h-5 text-emerald-600" /> Tawk.to Live Chat Support Integration
                  </h2>
                  <p className="text-xs text-slate-500 mt-1">Connect your official Tawk.to account to chat with traders, investors, and website visitors directly from the web or Tawk.to mobile app.</p>
                </div>
                <div className="flex items-center gap-2">
                  <button
                    type="button"
                    onClick={() => {
                      const opened = openTawkToChat();
                      if (opened) {
                        showToast('💬 Opening live Tawk.to chat widget on this page...', 'success');
                      } else {
                        showToast('Please enter and save your Tawk.to Property ID first.', 'error');
                      }
                    }}
                    className="bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold px-4 py-2 rounded-xl transition cursor-pointer flex items-center gap-1.5 shadow-sm"
                  >
                    <MessageSquare className="w-4 h-4" /> Test Chat Widget
                  </button>
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="bg-slate-900 hover:bg-slate-800 text-white text-xs font-bold px-4 py-2 rounded-xl transition cursor-pointer flex items-center gap-1.5 shadow-sm"
                  >
                    <ExternalLink className="w-4 h-4 text-emerald-400" /> Open Tawk.to Agent Console
                  </a>
                </div>
              </div>
              ...
              <div className={`p-4 rounded-2xl border flex flex-col md:flex-row items-start md:items-center justify-between gap-4 ${
                ...
                <div className="flex items-center gap-3">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center font-black text-sm shrink-0 ${
                    tawkConfig.enabled && tawkConfig.propertyId ? 'bg-emerald-500 text-white shadow-sm' : 'bg-amber-500 text-white shadow-sm'
                  }`}>
                    <MessageSquare className="w-5 h-5" />
                  </div>
                  <div>
                    <div className="flex items-center gap-2">
                      <h4 className="font-bold text-sm">
                        {tawkConfig.enabled && tawkConfig.propertyId ? 'Tawk.to Live Chat is ACTIVE on Website' : 'Tawk.to Setup In Progress'}
                      </h4>
                      <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded-full ${
                        tawkConfig.enabled && tawkConfig.propertyId ? 'bg-emerald-200 text-emerald-800' : 'bg-amber-200 text-amber-800 animate-pulse'
                      }`}>
                        {tawkConfig.enabled && tawkConfig.propertyId ? 'Online' : 'Pending Keys'}
                      </span>
                    </div>
                    <p className="text-xs text-slate-600 mt-0.5">
                      {tawkConfig.enabled && tawkConfig.propertyId
                        ? `Property: ${tawkConfig.propertyId} • Widget: ${tawkConfig.widgetId || 'default'} • Client context sync: Enabled`
                        : 'Enter your Property ID & Widget ID below to enable instant 2-way live chat with all website visitors.'}
                    </p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    onClick={() => {
                      const updated = { ...tawkConfig, enabled: !tawkConfig.enabled };
                      setTawkConfig(updated);
                      saveTawkToConfig(updated);
                      showToast(updated.enabled ? '✅ Tawk.to Live Chat enabled!' : '⏸️ Tawk.to Live Chat disabled.', 'info');
                    }}
                    className={`px-4 py-2 rounded-xl text-xs font-black transition cursor-pointer ${
                      tawkConfig.enabled ? 'bg-emerald-600 text-white hover:bg-emerald-700' : 'bg-slate-200 text-slate-800 hover:bg-slate-300'
                    }`}
                  >
                    {tawkConfig.enabled ? 'Disable Widget' : 'Enable Widget'}
                  </button>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                  ...
                  <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs space-y-5">
                    <div className="border-b border-slate-100 pb-3 flex items-center justify-between">
                      <h3 className="font-bold text-slate-900 text-base flex items-center gap-2">
                        ...
                        Tawk.to Credentials & Parameters
                      </h3>
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Auto-Saved</span>
                    </div>

                    {/* Quick Paste Snippet Parser */}
                    <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-2">
                      <label className="text-xs font-bold text-slate-700 block">
                        📋 Quick Paste: Tawk.to Embed Code Snippet
                      </label>
                      <p className="text-[11px] text-slate-500">
                        Paste your full script tag from your Tawk.to dashboard. We will automatically extract your Property ID and Widget ID:
                      </p>
                      <div className="flex gap-2">
                        <input
                          type="text"
                          value={tawkSnippetInput}
                          onChange={(e) => setTawkSnippetInput(e.target.value)}
                          placeholder="e.g. <script src='https://embed.tawk.to/64a.../1h...'></script>"
                          className="flex-1 border border-slate-300 rounded-lg p-2 text-xs font-mono bg-white outline-none focus:ring-2 focus:ring-emerald-500"
                        />
                        <button
                          type="button"
                          onClick={() => {
                            if (!tawkSnippetInput.trim()) {
                              showToast('Please paste a Tawk.to snippet or URL first.', 'error');
                              return;
                            }
                            // Regex parser for embed.tawk.to/PROPERTY_ID/WIDGET_ID
                            const match = tawkSnippetInput.match(/embed\.tawk\.to\/([a-zA-Z0-9_-]+)\/([a-zA-Z0-9_-]+)/);
                            if (match && match[1] && match[2]) {
                              const newConfig = {
                                ...tawkConfig,
                                propertyId: match[1],
                                widgetId: match[2],
                                directChatUrl: `https://tawk.to/chat/${match[1]}/${match[2]}`,
                                enabled: true
                              };
                              setTawkConfig(newConfig);
                              saveTawkToConfig(newConfig);
                              setTawkSnippetInput('');
                              showToast(`✅ Extracted Property ID: ${match[1]} & Widget ID: ${match[2]}!`, 'success');
                            } else {
                              showToast('Could not find Property ID in snippet. Please paste the full script tag or fill fields manually below.', 'error');
                            }
                          }}
                          className="bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold px-3 py-2 rounded-lg transition cursor-pointer shrink-0"
                        >
                          Auto-Extract
                        </button>
                      </div>
                    ...
                    <div className="space-y-4">
                      <div>
                        ...
                        <input
                          type="text"
                          value={tawkConfig.propertyId}
                          onChange={(e) => setTawkConfig({ ...tawkConfig, propertyId: e.target.value.trim() })}
                          placeholder="e.g. 64a1b2c3d4e5f6a7b8c9d0e1"
                          className="w-full border border-slate-300 rounded-lg p-2.5 text-xs font-mono bg-slate-50 focus:bg-white outline-none focus:ring-2 focus:ring-emerald-500 font-bold"
                        />
                        <p className="text-[10px] text-slate-500 mt-1">Found in your Tawk.to Dashboard under <strong>Administration → Channels → Chat Widget</strong></p>
                      </div>

                      <div>
                        <label className="text-xs font-bold text-slate-700 block mb-1">
                          Tawk.to Widget ID <span className="text-slate-400 font-normal">(Default is usually "default" or "1h1234567")</span>
                        </label>
                        <input
                          type="text"
                          value={tawkConfig.widgetId}
                          onChange={(e) => setTawkConfig({ ...tawkConfig, widgetId: e.target.value.trim() })}
                          placeholder="e.g. default or 1h1234567"
                          className="w-full border border-slate-300 rounded-lg p-2.5 text-xs font-mono bg-slate-50 focus:bg-white outline-none focus:ring-2 focus:ring-emerald-500 font-bold"
                        />
                      </div>
                      ...
                      <div>
                        <label className="text-xs font-bold text-slate-700 block mb-1">
                          Direct Chat URL <span className="text-slate-400 font-normal">(Optional standalone link)</span>
                        </label>
                        <input
                          type="text"
                          value={tawkConfig.directChatUrl || ''}
                          onChange={(e) => setTawkConfig({ ...tawkConfig, directChatUrl: e.target.value.trim() })}
                          placeholder="https://tawk.to/chat/64a.../1h..."
                          className="w-full border border-slate-300 rounded-lg p-2.5 text-xs font-mono bg-slate-50 focus:bg-white outline-none focus:ring-2 focus:ring-emerald-500"
                        />
                      </div>
                      ...
                      <div className="pt-2 border-t border-slate-100 space-y-3">
                        <label className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-200 cursor-pointer hover:bg-slate-100 transition">
                          <div>
                            <span className="text-xs font-bold text-slate-900 block">Enable Live Chat on Website</span>
                            <span className="text-[10px] text-slate-500">Injects and renders the Tawk.to live support bubble across all client and public pages.</span>
                          </div>
                          <input
                            type="checkbox"
                            checked={tawkConfig.enabled}
                            onChange={(e) => setTawkConfig({ ...tawkConfig, enabled: e.target.checked })}
                        ...
                        <label className="flex items-center justify-between p-3 bg-slate-50 rounded-xl border border-slate-200 cursor-pointer hover:bg-slate-100 transition">
                          <div>
                            <span className="text-xs font-bold text-slate-900 block">Auto-Open Chat Box on Visitor Entry</span>
                            <span className="text-[10px] text-slate-500">Automatically expands the chat window to greet first-time visitors and active traders.</span>
                          </div>
                          <input
                            type="checkbox"
                            checked={tawkConfig.autoOpen}
                            onChange={(e) => setTawkConfig({ ...tawkConfig, autoOpen: e.target.checked })}
                            className="w-5 h-5 text-emerald-600 rounded cursor-pointer"
                          />
                        </label>
                      </div>
                      ...
                      <div className="pt-4 flex items-center gap-3">
                        <button
                          ...
                          onClick={() => {
                            ...
                            setTimeout(() => {
                              setIsTawkSaving(false);
                              showToast('💾 Tawk.to configuration saved & activated on live site!', 'success');
                            }, 400);
                          }}
                          disabled={isTawkSaving}
                          className="bg-emerald-600 hover:bg-emerald-700 text-white font-extrabold text-xs px-6 py-3 rounded-xl flex items-center justify-center gap-2 shadow-md transition cursor-pointer flex-1"
                        >
                          {isTawkSaving ? (
                            <>
                            ...
                            <>
                              <CheckCircle2 className="w-4 h-4" /> Save & Activate Tawk.to on Website
                            </>
                          )}
                        </button>
                      </div>
                ...
                {/* Right Column: Information, Guides & Visitor Sync */}
                <div className="lg:col-span-5 space-y-6">
                  ...
                  <div className="bg-slate-900 text-white border border-slate-800 rounded-2xl p-6 shadow-sm space-y-4">
                    <div className="flex items-center gap-2 text-emerald-400 font-bold text-xs uppercase tracking-wider">
                      <Zap className="w-4 h-4" /> 3-Step Setup Instructions
                    </div>

                    <div className="space-y-3 text-xs">
                      <div className="flex items-start gap-3 bg-slate-800/80 p-3 rounded-xl border border-slate-700">
                        <span className="w-6 h-6 rounded-full bg-emerald-500/20 text-emerald-400 flex items-center justify-center font-bold text-xs shrink-0 border border-emerald-500/40">1</span>
                        <div>
                          <p className="font-bold text-white">Create Free Tawk.to Account</p>
                          <p className="text-[11px] text-slate-300 mt-0.5">
                            Sign up for free at <a href="https://www.tawk.to" target="_blank" rel="noreferrer" className="text-emerald-400 underline font-bold">tawk.to</a>. Zero subscription fee required.
                          </p>
                        </div>
                      </div>

                      <div className="flex items-start gap-3 bg-slate-800/80 p-3 rounded-xl border border-slate-700">
                        <span className="w-6 h-6 rounded-full bg-emerald-500/20 text-emerald-400 flex items-center justify-center font-bold text-xs shrink-0 border border-emerald-500/40">2</span>
                        <div>
                          <p className="font-bold text-white">Copy Property & Widget ID</p>
                          <p className="text-[11px] text-slate-300 mt-0.5">
                            Navigate to <strong>Administration ⚙️ → Channels → Chat Widget</strong>. Copy the Property ID or the Direct Chat Link.
                          </p>
                        </div>
                      </div>

                      <div className="flex items-start gap-3 bg-slate-800/80 p-3 rounded-xl border border-slate-700">
                        <span className="w-6 h-6 rounded-full bg-emerald-500/20 text-emerald-400 flex items-center justify-center font-bold text-xs shrink-0 border border-emerald-500/40">3</span>
                        <div>
                          <p className="font-bold text-white">Paste & Click Save</p>
                          <p className="text-[11px] text-slate-300 mt-0.5">
                            Paste into the fields on the left and click <strong>Save & Activate</strong>. The chat widget will appear immediately on the live website!
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="pt-2 border-t border-slate-800 flex justify-between items-center text-[11px] text-slate-400">
                      <span>📱 Receive chat notifications on iOS & Android apps</span>
                    </div>
                  </div>

                  {/* Trader Metadata Sync Card */}
                  <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-xs space-y-3">
                    <h3 className="font-bold text-slate-900 text-sm flex items-center gap-2">
                      <ShieldCheck className="w-4 h-4 text-emerald-600" />
                      Automatic Trader Metadata Synchronization
                    </h3>
                    <p className="text-xs text-slate-600 leading-relaxed">
                      When a registered trader initiates a chat on the website, their account details are automatically synced to the Tawk.to Agent Desk:
                    </p>

                    <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 space-y-1.5 font-mono text-[11px] text-slate-700">
                      <div className="flex justify-between">
                        <span className="text-slate-400">Visitor Name:</span>
                        <span className="font-bold text-slate-900">User's Full Legal Name</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-400">Email Address:</span>
                        <span className="font-bold text-slate-900">user@example.com</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-400">Live Account Number:</span>
                        <span className="font-bold text-indigo-600">AXI-MT5-882910</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-400">Live Balance ($):</span>
                        <span className="font-bold text-emerald-600">${liveBalance.toLocaleString()} USD</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-slate-400">Account Tier:</span>
                        <span className="font-bold text-slate-900">Pro ECN Prime</span>
                      </div>
                      <div className="flex justify-between">
