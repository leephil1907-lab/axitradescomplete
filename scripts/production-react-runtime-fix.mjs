...
// React #300: never conditionally skip hooks because of the email-action route.
...
if(app.includes(returnAnchor) && !app.includes('shouldRenderEmailAction) return <EmailActionPage')) app=app.replace(returnAnchor,returnReplacement);

// Standalone admin sessions must bypass the customer-auth redirect without creating a hook-order change.
...
if(app.includes(authAnchor) && !app.includes('hasStandaloneAdminSession')) app=app.replace(authAnchor,authReplacement);
