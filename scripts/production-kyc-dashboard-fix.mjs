...
function replaceRequired(source, from, to, label) {
  if (!source.includes(from)) throw new Error(`KYC fix could not find ${label}`);
...
const newBackend = `      const token = await currentUser.getIdToken();
      const response = await fetch('/api/kyc/submit', {
        ...
        body: JSON.stringify({
          id: \`KYC-\${currentUser.uid}-\${Date.now()}\`, fullName: fullName.trim(), dob, streetAddress, city,
          postalCode, country, docType, docNumber: docNumber.trim(), user: fullName.trim(),
          userEmail: currentUser.email || '', type: \`\${docType} & Proof of Address\`, documents,
          fileName: documents.map(document => document.fileName).join(', '),
          submittedAt: new Date().toISOString(), status: 'Pending', level: 1,
          refCode: \`DOC-\${Math.floor(100000 + Math.random() * 900000)}\`,
          details: { fullName: fullName.trim(), dob, streetAddress, city, postalCode, country, docType, docNumber: docNumber.trim() }
        })
      });
      const raw = await response.text();
      let result: any = {};
