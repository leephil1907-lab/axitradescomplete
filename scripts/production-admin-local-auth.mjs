...
// Production-only admin authentication. Regular client authentication remains Firebase.
...
// Add the standalone admin login endpoint without touching existing routes.
...
// Admin API helper: standalone admin bearer token wins over Firebase.
...
// Admin dashboard login gate. Inject only once.
...
`;
 admin=admin.replace(marker,marker+'\n'+gate);fs.writeFileSync('src/components/AdminDashboardView.tsx',admin);
}

// Restore the original hidden logo entry: seven rapid clicks reveal the private admin prompt.
...
if(!header.includes('AXI_HIDDEN_ADMIN_TRIGGER_V3')){
 ...
 const firstEnd=header.indexOf(logoEnd);
...
`;
 header=header.slice(0,firstEnd+logoEnd.length)+prompt+header.slice(firstEnd+logoEnd.length);
 fs.writeFileSync('src/components/Header.tsx',header);
}

// Allow the standalone admin session through App's client-auth guard.
