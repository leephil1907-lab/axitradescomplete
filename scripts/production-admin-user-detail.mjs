...
if(!s.includes('History, UserRound')){
  ...
  if(!iconLine.test(s))throw new Error('Dashboard icon import anchor missing');
  s=s.replace(iconLine,iconImport);
}
...
if(!s.includes('<AdminUserDetailDrawer userId={adminUserDetailId}')){
  const close='</main></div>;'; const injected='<AdminUserDetailDrawer userId={adminUserDetailId} onClose={()=>setAdminUserDetailId(null)} showToast={showToast}/>'; const idx=s.lastIndexOf(close); if(idx<0)throw new Error('Admin dashboard closing marker missing'); s=s.slice(0,idx)+injected+s.slice(idx);
}
